...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите ФИО пассажира');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое ФИО пассажира', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Пассажир '${taskName}' будет удален. Продолжить?`)) return;
  ...
  render() {
    ...
    button.innerHTML = 'Добавить пассажира';
